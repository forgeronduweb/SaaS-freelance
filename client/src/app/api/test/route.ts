...
// GET /api/test - Test de base de l'API et connexion Vercel
export async function GET() {
  try {
    ...
    return createSuccessResponse({
      message: 'API AfriLance fonctionnelle sur Vercel',
      database: 'Connectée',
      environment: process.env.NODE_ENV,
      vercel: process.env.VERCEL ? 'Déployé sur Vercel' : 'Environnement local',
      stats: {
        users: userCount,
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Erreur POST test API:', error)
    return handleApiError(error)
  }
}
