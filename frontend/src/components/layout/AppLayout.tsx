...
interface AppLayoutProps {
    ...
    userType?: 'freelance' | 'client' | 'guest';
}
    ...
    const getMenuItems = () => {
        if (userType === 'freelance') {
            ...
            return [
                ...
                { href: '/freelances', label: 'Freelances', icon: 'team' },
                { href: '/missions', label: 'Missions', icon: 'missions' },
                { href: '/messages', label: 'Messagerie', icon: 'messages' },
                { href: '/dashboard/freelance', label: 'Dashboard Freelance', icon: 'dashboard' },
                { href: '/dashboard/client', label: 'Dashboard Client', icon: 'dashboard' },
            ];
        }
    };
            ...
            <div className="flex">
                <aside className="hidden md:block fixed left-0 top-16 w-64 bg-white shadow-sm h-[calc(100vh-4rem)] overflow-y-auto z-40">
                    <nav className="p-4 flex flex-col h-full">
                        ...
                        <div className="mt-auto p-3 bg-orange-50 rounded-lg border border-orange-200">
                            <h2 className="font-semibold text-orange-800">{pageTitle}</h2>
                            {pageDescription && (
                                <p className="text-sm text-orange-600 mt-1">{pageDescription}</p>
                            )}
                        </div>
                    </nav>
                </aside>
                ...
                {isMobileMenuOpen && (
                    <>
                        ...
                        <aside className="md:hidden fixed left-0 top-16 w-80 bg-white shadow-lg h-[calc(100vh-4rem)] overflow-y-auto z-50">
                            <nav className="p-4 flex flex-col h-full">
                                <ul className="space-y-2 flex-1">
                                    {menuItems.map((item) => {
                                        const isActive = pathname === item.href;
